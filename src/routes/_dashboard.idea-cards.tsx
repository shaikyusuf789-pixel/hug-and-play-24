    ...
    <div className="mx-auto max-w-5xl space-y-8 p-6">
      <header className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900">Idea Cards</h1>
            <p className="text-sm font-medium text-slate-500 uppercase tracking-widest mt-0.5">Content Intelligence Pipeline</p>
          </div>
        </div>
        <Button
          onClick={() => refetch()}
          disabled={isFetching}
          variant="outline"
          className="h-11 gap-2 rounded-xl font-bold border-slate-200"
        >
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center gap-2.5 rounded-2xl px-5 py-3 text-xs font-bold uppercase tracking-wider transition-all border",
                active
            ...
            >
              ...
              {t.label}
              <span className={cn(
                "ml-1 rounded-lg px-2 py-0.5 text-[10px] font-black",
      ...
      <div className="grid gap-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-[3rem] bg-white border border-dashed border-slate-200 py-24 text-center">
            <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-slate-50 text-slate-300 mb-6">
              <Sparkles className="h-10 w-10" />
            </div>
            <h3 className="text-xl font-black text-slate-900">No {activeTab.toLowerCase()} ideas</h3>
            <p className="mt-2 text-slate-500 font-medium">Your {activeTab.toLowerCase()} queue is empty.</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2">
            {filtered.map((idea) => (
