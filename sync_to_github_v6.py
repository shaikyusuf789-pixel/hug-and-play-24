...
def sync_project():
    ...
    files_to_sync = [
        ...
        "railway-worker/workers/audio_engine.py", # If created
    ]
    
    # Also sync screenshots
    screenshot_dir = Path("docs/screenshots/v6")
    if screenshot_dir.exists():
        for screenshot in screenshot_dir.glob("*.png"):
            files_to_sync.append(str(screenshot))

    for file_path in files_to_sync:
        if os.path.exists(file_path):
            push_file_to_github(file_path, file_path, f"Sync {file_path} - v6.0 Biography update")
        else:
